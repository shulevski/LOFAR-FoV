...
function setUp(){
	...
	$.getJSON('/uvis/static/scripts/lofar.json', {_: new Date().getTime()}, function(response){
		 ...
		 setTimeout(setUp, 60000); // Refresh the display once a minute
}
...
function loadJSON(callback) {   
	...
	xobj.open('GET', '/uvis/static/scripts/lofar.json', true);
	xobj.send(null);
...
function submitTargets(){
        document.getElementById("sasID").submit();
}
